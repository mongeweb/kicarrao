...
<body>
    ...
    else
    {
        ...
        if($valorPlano > 0)
        {
            $_SESSION['planoValorRenovacao'] = $valorPlano;
        }
        else
        {
    ...
    if(isset($_GET['debug']))
        $_SESSION['planoValorRenovacao'] = 1;
    ?>
    ...
    function pagamento()
    {
        ...
        $.ajax(
        {
            type: "POST",
            //async: false,
            url: "<?=sslRedir().PROJECT_URL;?>/ajaxRenovacaoFinalizacao.php",
            dataType: "json",
            data:
            {
                "acao": _acao,
            ...
            if(json.erro == 0)
            {
                if(json.xml.status == 150)
                {
                    $("body").append("<form style='display: none;' id='formDebito' name='downloadForm' action='"+json.xml.acs_url+"' method='post'>"+ 
                                        ...
                                        "</textarea>"+
                                        "<input type='text' name='TermUrl' value='https://www.kicarrao.com.br/anuncio-renovacao-confirmacao.php'>"+
                ...
                {
                    //alert(json.msg);
                    document.location.replace('<?=$MG_MR_Settings['active'] ? sslRedir().PROJECT_URL.'/anuncio-renovacao-confirmacao' : sslRedir().PROJECT_URL.'/anuncio-renovacao-confirmacao.php';?>')
                    //closeLoad();
                }
            }
            else
                ...
                <div class="anunciewrap">
                    <div class="anunciedadosbloco">
                        <div class="anunciefinalbloco">
                            <!--<p class="anunciefinaltitulo">Seu cadastro foi realizado com sucesso!</p>-->
