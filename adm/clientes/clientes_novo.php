...
{    
    ...
    if($erro == 0)
    {
		...
		else
		{
            ...
            $ordem = $tipoAnuncio == '2' ? reordenarClientes() : 0;
            $cleanTitulo = mg_substituirEspacosPorTreacos($cleanTitulo);
            $sql = "INSERT INTO clientes (dataCadastro, tipoAnuncio, nomeFantasia, cleanTitulo, razaoSocialNome, sobrenome, documento, cep, endereco, numero, complemento, bairro, cidade, estado, pais, telRes, telCel, telCom, email, senha, tipoRevenda, codPlanoRevenda, responsavel, site, situacao, emailAtivacao, ordem)
                    VALUES(
                    '{$dataCadastro}', '{$tipoAnuncio}', '{$nomeFantasia}', '{$cleanTitulo}', '{$razaoSocialNome}', '{$sobrenome}', '{$documento}', '{$cep}', '{$endereco}', '{$numero}', '{$complemento}', '{$bairro}', '{$cidade}', '{$estado}', '{$pais}', '{$telRes}', '{$telCel}', '{$telCom}', '{$email}', '{$senhacript}', '{$tipoRevenda}', '{$codPlanoRevenda}', '{$responsavel}', '{$site}', '{$situacao}', '{$emailAtivacao}', '{$ordem}');";
    		...
    		if($fotoBanner != '')
            {
        		...
        		$sqlG = "INSERT INTO arquivos (referencia, codReferencia,tipo, arquivo, codigo)
                        VALUES ('bannerRevenda', '{$codRef}','1', '{$fileG}','$token')";
        		for($a=0;$a<5;$a++)
                {
                    $resultadoG = mysql_query($sqlG);
                    if($resultadoG)
...
<div id="contentFormNovoEdita" style="width: 100%;">
    ...
    <fieldset>
        ...
        <div>
            <label for="documento"><span class="obrigatorio">*</span> Documento: </label>
            <input type="text" name="documento" id="documento" title="Documento" value="<?=remEscape($documento);?>" />
        </div>
        <div>
        ...
        <div style="display:none;" class="tipoRevenda">
            ...
            <div>
                ...
                <select name="codPlanoRevenda" id="codPlanoRevenda" title="Plano da revenda">
                    ...
                    <?php
                    	$sqlPlanoRev = mysql_query("SELECT * FROM planosRevenda ORDER BY cod ASC");
                    ...
                    ?>
                            <option value="<?=$tpPlanoRev['cod'];?>" <?=$tpPlanoRev['cod'] == $codPlanoRevenda ? "selected = 'selected'" : "";?>>
								<?=$tpPlanoRev['plano'];?> - <?=$tpTipoVeiculo['tipoVeiculo'];?>
							</option>
                    <?php
                    	}
                    ?>
            ...
            <div>
            	<label for="imagem">Logo da Empresa: </label>
                <input type="file" name="file" id="file" title="Foto" />
        	</div>
            <div>
            	<label for="imagem">Banner: </label>
                <input type="file" name="fileBanner" id="fileBanner" title="Banner" />
        	</div>
        </div>
        <div>
