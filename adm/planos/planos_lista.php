...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest'))
{
    ...
    $sql = "UPDATE planosParticular SET $campo = '$valor' WHERE cod = '$codPlano'";
    $erro = 0;
    for($a=0;$a<5;$a++)
    {
        $q = mysql_query($sql);
...
require_once ADMIN_TOP_PATH.'planos_topo.php';
?>
    ...
    _ajax = $.ajax(
            {
                type: "POST",
                async: false,
                url: "http://<?=ADMIN_URL;?>/planos/planos_lista.php", //URL de destino
                data:
                {
                    cod: _cod,
                    valor: _val,
        ...
        if ($nPlanos>0)
        {
        	?>
                ...
                <tr rel='destaqueCapa' class="tr <?=($a/2) == ceil($a/2) ? 'linhacinza' : '';?>">
                    <td>Destaque na capa do site</td>
                    <?php
                    foreach($planos as $k => $val)
    	            {
                    ?>
                        <td><input class="input" type="checkbox" <?=$val['destaqueCapa'] == 1 ? 'checked' : '';?> value="1" title="Destaque na capa do site/Plano <?=$val['plano'];?>" rel="<?=$k;?>" /></td>
                    <?php
                    }
                    $a++;
                    ?>
                ...
                <tr rel='destaqueLista' class="tr <?=($a/2) == ceil($a/2) ? 'linhacinza' : '';?>">
                    <td>Destaque na lista de resultados</td>
                    <?php
                    foreach($planos as $k => $val)
    	            {
                    ?>
                ...
                <tr rel='video' class="tr <?=($a/2) == ceil($a/2) ? 'linhacinza' : '';?>">
                    <td>Vídeo</td>
                    <?php
                    foreach($planos as $k => $val)
    	            {
                    ?>
                        <td><input class="input" type="checkbox" <?=$val['video'] == 1 ? 'checked' : '';?> value="1" title="Vídeos/Plano <?=$val['plano'];?>" rel="<?=$k;?>" /></td>
                    <?php
                    }
                    $a++;
                    ?>
