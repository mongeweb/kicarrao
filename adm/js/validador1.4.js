...
function xform(obj, settings)
{
    ...
    if ($(obj).size() > 0) {
        ...
        this . addErro('Formul\u00E1rio inv\u00E1lido!');
        this . status = false;
    }
	...
	{
        var invaidfield = false;
        if (typeof(campo) == 'string') {
            //alert('string: ' + campo);
            reg = new RegExp(this.formId); 
            if (!campo.match(reg)) {
                campo = 'form#'+this.formId+' '+campo;
            }
            if ($(campo).size() <= 0) {
                invaidfield = true;
            }
        ...
        if (typeof(campo) == 'object') {
            ...
            if (campo == null || !campoId) {
                invaidfield = true;
                //alert('object: ' + campo + ' e nulo');
            }
        }
        //alert(typeof(campo));
        if (invaidfield) {
        ...
        for (var y = 0; y < ixx . length; y++) {
            ...
            if (ixx[y] . campo . length > 0 && ixx[y] . campo[0]) {
                if (ixx[y] . campo[0] . type == 'radio') {
                    ...
                    var campoValue = $(ixx[y] . campo).filter(':checked').val();
                }
    		...
    		if (!branco(campoValue)) {
    	        ...
    	        // teste para radio button
                if (ixx[y] . campo . length > 0 && ixx[y] . campo[0] . type == 'radio') {
                    var i = $(ixx[y] . campo).find(':checked').size();
    				if (i < 0) {
