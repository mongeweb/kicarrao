...
{   
    ...
    {		
			...
			for ($x=0;$x<count($ordem);$x++)
            {
				$i = $x+1;
				$refer = $ordem[$x];
				$varUpt = "UPDATE guiaAutomotivo SET ordem = '$i' WHERE cod = '$refer'";
...
$resultado = mysql_query("SELECT * FROM guiaAutomotivo ORDER BY ordem ASC");	 
...
<script type="text/javascript">
	$(document).ready(function()
    {
		...
		$("#Salvar").click(function()
        {
			...
			$.post("http://<?=ADMIN_URL;?>/guiaAutoMotivo/guiaAutoMotivo_ordena.php",
            {
               data : ordem
            }
            ,
            function()
            {
            	...
            	location.replace("http://<?=ADMIN_URL;?>/principal.php?id=3&subid=1");
            });
		})
	})
</script>
...
if ($linhas>0)
{
	for($n=0;$n<$linhas;$n++)
	{
        ...
        $titulo = $tupla['empresa'];
		if (strlen($titulo) > 100)
			$nome = substr($titulo,0,100)."..."; 
		else
			$nome = $titulo;
			
		echo $tupla['ordem'] ."  -  ". $nome;  
        echo "</li>";		
	}
}
else
...
<div style="width:790px;">
    <a class="botaoDourado" href="http://<?=ADMIN_URL;?>/principal.php?id=3&subid=1" style="float:left;width: 50px;text-align: center;">Voltar</a>
